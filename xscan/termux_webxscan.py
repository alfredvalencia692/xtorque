...
def classify_threat(scan_data: Dict[str, Any]) -> str:
    ...
    suspicious = vt.get("suspicious", 0) if isinstance(vt, dict) else 0

    ssl_info = scan_data.get("ssl_info", {})
    ssl_error = isinstance(ssl_info, dict) and ssl_info.get("error")
    ...
    if malicious > 0:
        return "Dangerous"
    if ssl_error and ("hostname" in str(ssl_error).lower() or "certificate" in str(ssl_error).lower()):
        return "Dangerous"
    if suspicious > 0 or missing_headers >= 3:
        return "Risky"
...
def assess_confidence_and_recommendation(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if total > 0:
        threat_ratio = (malicious * 2 + suspicious) / total
    ...
    if score >= 80:
        ...
        recommendation = "⚠️ Moderate Risk — Exercise caution."
        color = "yellow"
    elif score >= 40:
        recommendation = "⚠️ High Risk — Significant concerns detected."
        color = "orange"
    else:
        recommendation = "🚨 Critical Risk — Avoid this target!"
        color = "red"
...
<body>
    ...
    <script>
        ...
        function displayResults(data) {
            ...
            const statusClass = threat.toLowerCase().includes('dangerous') ? 'status-danger' :
                              threat.toLowerCase().includes('risky') || threat.toLowerCase().includes('suspicious') ? 'status-warn' :
                              'status-safe';
